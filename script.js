...
const foodURL =
    "https://dataportal.livsmedelsverket.se/livsmedel/api/v1/livsmedel?limit=2000&offset=0";

let nutritionURL =
    "https://dataportal.livsmedelsverket.se/livsmedel/api/v1/livsmedel/{nummer}/naringsvarden";
...
function renderIngredientList() {
    ...
    selectedIngredients.forEach((item) => {
        ...
        setTimeout(() => {
            //runs the code when the callstack is empty
...
async function getNutrition(id) {
    ...
    return data
        .filter((n) => allowed.includes(n.namn))
        .map((n) => ({
            name: n.namn,
            value: n.varde,
            unit: n.enhet,
...
function calculateTotals() {
    ...
    for (let item of selectedIngredients) {
        ...
        for (let n of nutrition) {
            // check loop through each nutrition for the item
